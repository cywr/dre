import { log as logMsg, LogType } from "./logger"
...
export namespace DexExtractor {
  ...
  const log = (message: string) => logMsg(LogType.Info, NAME, message)
  ...
  export function getDexInfo(
    data: any,
  ): { version: string; fileSize: number; checksum: number } | null {
    try {
      const buffer = Java.array("byte", data)
 ...
 - Use 'pnpm run pull-dex' to copy files to local _build folder`)
   ...
   */
  export function processAndExtract(
    data: any,
    context: string,
    operation: string,
  ): {

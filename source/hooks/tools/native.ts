...
export namespace Native {
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    try {
                        ...
                        retryCount++;
                        Logger.log(Logger.Type.Debug, NAME, `Library '${libraryName}' not loaded yet, retrying in ${retryInterval}ms (attempt ${retryCount})`);
                        setTimeout(() => {
    ...
    export function attach(config: HookConfig): void {
        try {
            ...
            try {
                ...
                throw new Error(`Export lookup failed for ${config.library}::${config.method}: ${findError}`);
            }
            ...
            try {
                ...
                log(`✓ Hook ready for ${config.library}::${config.method}`);
            } catch (attachError) {
                throw new Error(`Interceptor.attach failed for ${config.library}::${config.method}: ${attachError}`);
            }
        } catch (error) {
     ...
     */
    export function call(library: string, method: string, returnType: string = 'pointer', argTypes: string[] = [], args: any[] = []): any {
        ...
        log(`Calling ${library}::${method}(${args.join(', ')}) -> ${returnType}`);
        ...
        log(`${library}::${method} returned: ${result}`);
        return result;
    }
     ...
     */
    export function callAndDump(library: string, method: string, dumpSize: number = 256, argTypes: string[] = [], args: any[] = []): any {
        try {
            const result = call(library, method, 'pointer', argTypes, args);
            
            if (result && !result.isNull()) {
                log(`${method} memory dump (${dumpSize} bytes): \n \n${hexDump(result, dumpSize)}\n `);
            } else {
                log(`${method} returned NULL pointer`);
            }
            return result;
        } catch (error) {
            Logger.log(Logger.Type.Error, NAME, `callAndDump failed: ${error}`);
            throw error;
        }
    }
    ...
    export function listExports(libraryName: string): string[] {
        ...
        const exportNames = exports.map(exp => exp.name).filter((name: string | null) => name !== null);
        
        log(`Found ${exportNames.length} exports in ${libraryName}`);
        log(`Exports: ${exportNames.slice(0, 10).join(', ')}${exportNames.length > 10 ? '...' : ''}`);
    ...
    export function listImports(libraryName: string): string[] {
        ...
        const importNames = imports.map(imp => imp.name).filter((name: string | null) => name !== null);
        
        log(`Found ${importNames.length} imports in ${libraryName}`);
        log(`Imports: ${importNames.slice(0, 10).join(', ')}${importNames.length > 10 ? '...' : ''}`);
    ...
    export function findExports(libraryName: string, pattern: string): string[] {
        try {
            ...
            log(`Found ${matches.length} exports matching '${pattern}' in ${libraryName}: ${matches.join(', ')}`);
            return matches;
        } catch (error) {
            Logger.log(Logger.Type.Error, NAME, `Failed to search exports in '${libraryName}': ${error}`);
            return [];
        }
    }
}